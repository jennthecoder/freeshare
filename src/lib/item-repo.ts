...
export const itemRepo = {
  findById(id: string, currentUserId?: string): Item | null {
    const row = get<any>(`
    ...
    `, [id]);
    if (!row) return null;
    const saved = currentUserId ? get<any>('SELECT 1 FROM saved_items WHERE user_id = ? AND item_id = ?', [currentUserId, id]) : null;
    return this.mapRow(row, !!saved);
  },

  findAll(filters: ItemFilters, currentUserId?: string): { items: Item[]; total: number } {
    let where = "WHERE 1=1";
    const params: any[] = [];
    ...
    const countRow = get<any>(`SELECT COUNT(*) as total FROM items i ${where}`, params);
    ...
    const savedIds = currentUserId ? new Set(all<any>('SELECT item_id FROM saved_items WHERE user_id = ?', [currentUserId]).map(r => r.item_id)) : new Set();
  ...
  create(userId: string, data: ItemCreate): Item {
    const id = generateId();
    const now = new Date().toISOString();
    run(`INSERT INTO items (id, user_id, title, description, images, category, condition, location_lat, location_lng, city, zip, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'available', ?, ?)`,
  ...
  update(id: string, userId: string, data: ItemUpdate): Item | null {
    const existing = get<any>('SELECT user_id FROM items WHERE id = ?', [id]);
    ...
    vals.push(id);
    run(`UPDATE items SET ${sets.join(', ')} WHERE id = ?`, vals);
    return this.findById(id);
  },
  ...
  saveItem(userId: string, itemId: string): boolean {
    try {
      run('INSERT OR IGNORE INTO saved_items (id, user_id, item_id, created_at) VALUES (?, ?, ?, ?)', [generateId(), userId, itemId, new Date().toISOString()]);
      return true;
    } catch { return false; }
  },

  unsaveItem(userId: string, itemId: string): boolean {
    run('DELETE FROM saved_items WHERE user_id = ? AND item_id = ?', [userId, itemId]);
    return true;
  },

  getSavedItems(userId: string, limit = 20, offset = 0): { items: Item[]; total: number } {
    const countRow = get<any>('SELECT COUNT(*) as total FROM saved_items WHERE user_id = ?', [userId]);
    const rows = all<any>(`
      SELECT i.*, u.id as user_id, u.name as user_name, u.avatar as user_avatar
